...
### 1. Polynomially Adjusted Gamma (PAM)
...
| 방법 | 공식 |
|------|------|
| ER1 (Easton-Ronchetti) | $K_H(x) \approx \sum_{i=1}^{4} \kappa_i x^i / i!$ |
| ER2 | $K_H(x) \approx \kappa_1 x + \frac{\kappa_2}{2}x^2 + \log(1 + \frac{\kappa_3}{6}x^3 + \frac{3\kappa_4}{72}x^4 + \frac{\kappa_3^2}{72}x^6)$ |
...
이산 분포의 특성을 고려한 연속성 보정 버전도 제공됩니다 (SDC1, SDC2).
...
├── saddlepoint.py        # 새들포인트 근사
├── pam.py                # 다항식 조정 감마 근사
├── gram_charlier.py      # Gram-Charlier 급수 근사
├── exact.py              # 정확 분포 (소표본)
...
# Saddlepoint
sp = SaddlepointApproximation(sample_sizes, cgf_method='ER1')
print(f"Saddlepoint: {sp.tail_probability_lr(4.62):.6f}")
...
논문의 Table 4.1 ($n_1=3, n_2=3, n_3=3$, $H=4.62222$, 유의수준 10%):

| 방법 | 논문 값 |
|------|---------|
| Exact (E-P) | 0.0842833 |
| Saddlepoint (SD1) | 0.0789061 |
| Saddlepoint CC (SDC1) | 0.124486 |
| Gram-Charlier (GC-A) | 0.396732 |
| PAM (degree 4) | 0.098122 |
| PAM (degree 6) | 0.0933829 |
...
1. **H 통계량 계산** (`kruskal_wallis.py:109`): 논문의 공식과 일치
2. **PAM 방법** (`pam.py`):
   - 감마 파라미터 추정: 정확
   - 모멘트 행렬 구성: 정확
...
4. **Gram-Charlier** (`gram_charlier.py`):
   - Hermite 다항식: 정확
   - 급수 전개 공식: 정확
5. **정확 분포** (`exact.py`):
